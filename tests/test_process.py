...
COLUMNS = ["Inbox (Goals)", "Needs Refinement", "Ready", "In Progress", "QAing", "Done"]
...
def test_rework_backwards_is_always_allowed(rules):
    assert rules.may_move(frm="QAing", to="In Progress", counts={}).allowed
    assert rules.may_move(frm="QAing", to="Needs Refinement", counts={}).allowed
...
def test_unblocking_returns_to_the_flow_without_a_skip_complaint(rules):
    assert rules.may_move(frm="Blocked", to="QAing", counts={}).allowed
...
def test_wip_limits_apply_to_rework_too(rules):
    ...
    v = rules.may_move(frm="QAing", to="In Progress", counts={"In Progress": 3})
    assert not v.allowed
    assert v.refusal is Refusal.WIP_LIMIT
